...
export default function CartMenu() {
  ...
  const total = cartItems.reduce((totalPrice, item) => {
    if (item.attributes != undefined) {
      var tempPrice = item.attributes.price == undefined ? 0 : item.attributes.price;
      return totalPrice + item.count * parseFloat(tempPrice);
    }
  ...
  var myView = cartItems.map((item) => (
    <tr key={item.id}>
      <td>
        <img
          width={100}
          src={AppUrl.ApiURL + (item.attributes != undefined ? item.attributes.image.data[0].attributes.url : '#st')}
          alt='tam'
        />
      </td>
      <td>{item.attributes != undefined ? item.attributes.productName : ''}</td>
      <td> - </td>
      <td>
      ...
      <td>
        {currency(item.attributes != undefined ? item.attributes.price : '0', {
          symbol: 'vnđ',
          separator: '.',
          decimal: ',',
        }).format()}
      ...
      <td>
        {currency((item.attributes != undefined ? item.attributes.price : 0) * item.count, {
          symbol: 'vnđ ',
          separator: '.',
          decimal: ',',
        }).format()}
